...
public class Scraper {
	...
	public JSONArray scrape(String url)
		...
		try {
			...
			logger.log(Level.INFO, "using puppeteer");
	...
	public String getHtml(String url) throws FourZeroFourException {
		try {
			...
			if (status.getStatusCode() == 404) {
				logger.severe(url + " produced a 404");
				throw new FourZeroFourException(url);
			}
			logger.severe(url + " produced a " + status.getStatusCode());
		} catch (IOException e) {
			logger.severe(url + " produced a " + e.getMessage());
			System.out.println(url + " produced a " + e.getMessage());
		} catch (IllegalArgumentException e1) {
			logger.severe(url + " produced a " + e1.getMessage());
	...
	public Model processTriples(String n3, IRI sourceIRI, Long contextCounter) {
		...
		try {
			...
			logger.severe("Cannot parse n3 into a model");
			return null;
		}
	...
	public String injectId(String html, String url)
		...
		if (posContext == -1) {
			...
			if (html.indexOf("@id") != -1) {
				...
				// should never happen
				logger.fine(url + " did not receive any injection! posContext is " + posContext);
